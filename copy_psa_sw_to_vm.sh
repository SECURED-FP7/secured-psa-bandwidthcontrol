...
IMG="/var/lib/libvirt/images/bandwidthControl.img"
SW_PATH="/home/psa/pythonScript/"

# Copy python files
echo -n "copy PSA SW... "
virt-copy-in -a $IMG PSA/psa/pythonScript/* $SW_PATH
echo "done."

# Copy interfaces file
echo -n "copy interfaces... "
virt-copy-in -a $IMG PSA/psa/pythonScript/interfaces /etc/network/
echo "done."

# Copy boot script that is executed when interfaces are up
echo -n "copy boot_script_psa... "
virt-copy-in -a $IMG PSA/psa/pythonScript/boot_script_psa /etc/network/if-up.d/
echo "done."
